...
ssh -i "infra\mcp-server-key.pem" ec2-user@mcp.domoticore.co "cd ~/MCP-SERVER && docker compose run --rm supervisor node dist/supervisor.js --once 2>&1"
```
...
```powershell
ssh -i "infra\mcp-server-key.pem" ec2-user@mcp.domoticore.co "grep -E 'indexSharedDirs completed|one-time complete' ~/index-cycle.log"
ssh -i "infra\mcp-server-key.pem" ec2-user@mcp.domoticore.co "grep 'Error fatal' ~/index-cycle.log"
```
...
```powershell
ssh -i "infra\mcp-server-key.pem" ec2-user@mcp.domoticore.co "curl -s http://localhost:6333/collections/mcp_docs" | findstr points_count
```
